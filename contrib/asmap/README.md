...
be used in Blokecoin Core with the `-asmap` option.
...
to binary before being used with Blokecoin Core.
...
the `blokecoin-cli getnodeaddresses` command.
...
```
blokecoin-cli getnodeaddresses 0 > addrs.json
```
