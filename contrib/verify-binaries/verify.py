...
"""Script for verifying Blokecoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from blokecoincore.org and blokecoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/blokecoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://blokecoincore.org"
HOST2 = "https://blokecoin.org"
VERSIONPREFIX = "blokecoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Blokecoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"blokecoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by blokecoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the blokecoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes blokecoin.org lags behind blokecoincore.org.)')
    )
