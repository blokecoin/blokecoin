...
complete --command blokecoin-cli --no-files

function __fish_blokecoin_cli_get_commands_helper
...
function __fish_blokecoin_cli_get_commands
    argparse 'nohelp' 'commandsonly' -- $argv
    set --local commands

    # Exclude description, exclude help
    if set -q _flag_nohelp; and set -q _flag_commandsonly
        set --append commands (__fish_blokecoin_cli_get_commands_helper | string replace -r ' .*$' '' | string match --invert -r 'help')
    # Include description, exclude help
    else if set -q _flag_nohelp
        set --append commands (__fish_blokecoin_cli_get_commands_helper | string replace ' ' \t | string match --invert -r 'help')
    # Exclude description, include help
    else if set -q _flag_commandsonly
        set --append commands (__fish_blokecoin_cli_get_commands_helper | string replace -r ' .*$' '')
    # Include description, include help
    else
        set --append commands (__fish_blokecoin_cli_get_commands_helper | string replace ' ' \t)
    end
...
function __fish_blokecoin_cli_get_options
...
complete \
    --command blokecoin-cli \
    --no-files \
    --condition "not __fish_seen_subcommand_from (__fish_blokecoin_cli_get_commands --commandsonly)" \
    --arguments "(__fish_blokecoin_cli_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete --command blokecoin-cli \
...
complete \
    ...
    --no-files \
    --condition "not __fish_seen_subcommand_from (__fish_blokecoin_cli_get_commands --commandsonly)" \
    --arguments "(__fish_blokecoin_cli_get_options --nofiles)"

# Add commands
# Permit command completions after `blokecoin-cli help` but not after other commands
complete \
    --command blokecoin-cli \
    --no-files \
    --condition "not __fish_seen_subcommand_from (__fish_blokecoin_cli_get_commands --commandsonly --nohelp)" \
    --arguments "(__fish_blokecoin_cli_get_commands)"
