...
complete --command blokecoin-qt --no-files

# Extract options
function __fish_blokecoinqt_get_options
...
complete \
    --command blokecoin-qt \
    --arguments "(__fish_blokecoinqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c blokecoin-qt \
...
complete \
    --command blokecoin-qt \
    --arguments "(__fish_blokecoinqt_get_options --nofiles)"
