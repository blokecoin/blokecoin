...
Provides utilities for managing copyright headers of `The Blokecoin Core
...
gen-blokecoin-conf.sh
===================

Generates a blokecoin.conf file in `share/examples/` by parsing the output from `blokecoind --help`. This script is run during the
release process to include a blokecoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-blokecoin-conf.sh
```
