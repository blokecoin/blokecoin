#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Blokecoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing blokecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='blokecoin-cli', help='blokecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Blokecoin address to which the faucet should send')
...
parser.add_argument('blokecoin_cli_args', nargs='*', help='Arguments to pass on to blokecoin-cli (default: -signet)')

args = parser.parse_args()

if args.blokecoin_cli_args == []:
    args.blokecoin_cli_args = ['-signet']


def blokecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.blokecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = blokecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = blokecoin_cli(['getnewaddress', 'faucet', 'bech32'])
