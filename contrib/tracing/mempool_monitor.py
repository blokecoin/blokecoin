#!/usr/bin/env python3
# Copyright (c) 2022 The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Example logging Blokecoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into blokecoind with pid {pid}")
    blokecoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    blokecoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    blokecoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    blokecoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    blokecoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[blokecoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of blokecoind>")
        exit(1)
