...
Tracing (USDT) support for Blokecoin Core on Linux using. For more information on
USDT support in Blokecoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `blokecoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `blokecoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Blokecoin Core USDT documentation] on how to list available tracepoints in your
`blokecoind` binary.

[Blokecoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof blokecoind)
```
...
Blokecoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof blokecoind)
```
...
example, a blokechain re-index. Based on the `validation:block_connected` USDT
tracepoint.
...
In a different terminal, starting Blokecoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/blokecoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof blokecoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof blokecoind)
```
