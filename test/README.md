This directory contains integration tests that test blokecoind and its
...
blokecoind and blokecoin-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, Blokecoin Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the blokecoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another blokecoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its blokecoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other blokecoind processes are running.

On linux, the test framework will warn if there is another
blokecoind process running when the tests are started.

If there are zombie blokecoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
blokecoind processes running on the system, so should not be used if any non-test
blokecoind processes are being run.**
...
A pre-mined blokechain with 200 blocks is generated the first time a
functional test is run and is stored in build/test/cache. This speeds up
test startup times since new blokechains don't need to be generated for
each test. However, the cache may get into a bad state, in which case
tests will fail. If this happens, remove the cache directory (and make
sure blokecoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and blokecoind `debug.log`s will all be dumped
...
`test_framework.log` and blokecoind `debug.log`s can be combined into a single
...
call methods that interact with the blokecoind nodes-under-test.

If further introspection of the blokecoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/blokecoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/blokecoind <pid>
```
