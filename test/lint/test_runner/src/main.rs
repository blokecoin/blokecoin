...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from blokecoin-build-config.h is used without the header being included",
...
fn lint_std_filesystem() -> LintResult {
    let found = git()
        .args([
            ...
            ":(exclude)src/blokecoin-chainstate.cpp",
        ])
        .status()
        .expect("command error")
        .success();
...
fn get_pathspecs_exclude_whitespace() -> Vec<String> {
    ...
    list.extend(
        [
            ...
            "contrib/init/blokecoind.openrc",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/blokecoin-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <blokecoin-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the blokecoin-build-config.h header. However, they are not
...
even though blokecoin-build-config.h indicates that a faster feature is available and should be used.
...
#include <blokecoin-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the blokecoin-build-config.h header. However, they are including
