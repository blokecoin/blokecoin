...
#
# Be aware that blokecoind and blokecoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in blokecoind.
#
...
# C locale.
#
# In contrast, blokecoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where blokecoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of blokecoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in blokecoind but
# "1,230000" in blokecoin-qt.
#
