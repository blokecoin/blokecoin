...
class PruneTest(BlokecoinTestFramework):
    ...
    def test_rescan_blokechain(self):
        self.restart_node(0, ["-prune=550"])
        assert_raises_rpc_error(-1, "Can't rescan beyond pruned data. Use RPC call getblokechaininfo to determine your pruned height.", self.nodes[0].rescanblokechain)
    ...
    def manual_test(self, node_number, use_timestamp):
        ...
        assert_raises_rpc_error(-1, "Cannot prune blocks because node is not in prune mode", node.pruneblokechain, 500)
        ...
        def prune(index):
            ret = node.pruneblokechain(height=height(index))
            assert_equal(ret + 1, node.getblokechaininfo()['pruneheight'])

        def has_block(index):
            return os.path.isfile(os.path.join(self.nodes[node_number].blocks_path, f"blk{index:05}.dat"))

        # should not prune because chain tip of node 3 (995) < PruneAfterHeight (1000)
        assert_raises_rpc_error(-1, "Blokechain is too short for pruning", node.pruneblokechain, height(500))
        ...
        assert_equal(node.getblokechaininfo()["blocks"], 1001)

        # prune parameter in the future (block or timestamp) should raise an exception
        future_parameter = height(1001) + 5
        if use_timestamp:
            assert_raises_rpc_error(-8, "Could not find block with at least the specified timestamp", node.pruneblokechain, future_parameter)
        else:
            assert_raises_rpc_error(-8, "Blokechain is shorter than the attempted prune height", node.pruneblokechain, future_parameter)

        # Pruned block should still know the number of transactions
        assert_equal(node.getblockheader(node.getblockhash(1))["nTx"], block1_details["nTx"])

        # negative heights should raise an exception
        assert_raises_rpc_error(-8, "Negative block height", node.pruneblokechain, -10)
    ...
    def run_test(self):
        ...
        self.log.info("Mining a big blokechain of 995 blocks")
    ...
    def test_pruneheight_undo_presence(self):
        node = self.nodes[5]
        pruneheight = node.getblokechaininfo()["pruneheight"]
        ...
        new_pruneheight = node.getblokechaininfo()["pruneheight"]
