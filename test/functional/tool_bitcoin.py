#!/usr/bin/env python3
# Copyright (c) The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test the blokecoin wrapper tool."""
from test_framework.test_framework import (
    BlokecoinTestFramework,
    SkipTest,
)
...
class ToolBlokecoinTest(BlokecoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `blokecoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `blokecoind` and `blokecoin-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/blokecoin/blokecoin/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through blokecoin wrapper command with specified arguments."""
        node.args = [self.binary_paths.blokecoin_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure blokecoin node command invokes blokecoind by default")
        self.test_args([], [], expect_exe="blokecoind")

        self.log.info("Ensure blokecoin -M invokes blokecoind")
        self.test_args(["-M"], [], expect_exe="blokecoind")

        self.log.info("Ensure blokecoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure blokecoin -m invokes blokecoin-node")
            self.test_args(["-m"], [], expect_exe="blokecoin-node")

            self.log.info("Ensure blokecoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="blokecoin-node")

            self.log.info("Ensure blokecoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="blokecoin-node")

            self.log.info("Ensure blokecoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="blokecoin-node")
...
if __name__ == '__main__':
    ToolBlokecoinTest(__file__).main()
