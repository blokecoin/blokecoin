#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Blokecoin Core developers
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"blokecoin-cli -h\" for help or \"blokecoin-cli listwallets\" to see "
...
class TestBlokecoinCli(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblokechaininfo RPC and `blokecoin-cli getblokechaininfo`")
        cli_response = self.nodes[0].cli.getblokechaininfo()
        rpc_response = self.nodes[0].getblokechaininfo()
        ...
        # Prevent blokecoin-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo returns expected network and blokechain info")
        ...
        blokechain_info = self.nodes[0].getblokechaininfo()
        assert_equal(int(cli_get_info['Version']), network_info['version'])
        assert_equal(cli_get_info['Verification progress'], "%.4f%%" % (blokechain_info['verificationprogress'] * 100))
        assert_equal(int(cli_get_info['Blocks']), blokechain_info['blocks'])
        assert_equal(int(cli_get_info['Headers']), blokechain_info['headers'])
        ...
        assert_equal(Decimal(cli_get_info['Difficulty']), blokechain_info['difficulty'])
        assert_equal(cli_get_info['Chain'], blokechain_info['chain'])

        self.log.info("Test -getinfo and blokecoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and blokecoin-cli getwalletinfo return expected wallet info")
            ...
            # Test blokecoin-cli -generate with -rpcwallet in multiwallet mode.
...
if __name__ == '__main__':
    TestBlokecoinCli(__file__).main()
