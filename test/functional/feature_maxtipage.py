...
class MaxTipAgeTest(BlokecoinTestFramework):
    ...
    def test_maxtipage(self, maxtipage, set_parameter=True, test_deltas=True):
        ...
        if test_deltas:
            ...
            for delta in [5, 4, 3, 2, 1]:
                ...
                assert_equal(node_ibd.getblokechaininfo()['initialblockdownload'], True)

        # tip within maximum age -> leave IBD
        node_miner.setmocktime(max(cur_time - maxtipage, 0))
        self.generate(node_miner, 1)
        assert_equal(node_ibd.getblokechaininfo()['initialblockdownload'], False)
