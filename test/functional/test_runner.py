...
`test/functional/test_framework/test_framework.py:BlokecoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_blokecoin_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_blokecoin_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
    ...
    'rpc_decodescript.py',
    'rpc_blokechain.py --v1transport',
    'rpc_blokechain.py --v2transport',
...
def main():
    ...
    enable_blokecoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_blokecoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if blokecoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "blokecoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a blokecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Blokecoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/blokecoin/blokecoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `blokecoin-cli help` (`rpc_interface.txt`).
