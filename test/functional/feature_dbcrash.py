...
class ChainstateWriteCrashTest(BlokecoinTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If blokecoind exits, then try again.  wait_for_node_exit()
                # should raise an exception if blokecoind doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, blokecoind isn't coming back up on restart.  Could be a
        # bug in blokecoind, or we've gotten unlucky with our dbcrash ratio --
