...
# Copyright 2016-present The Blokecoin Core developers
...
class ToolUtils(BlokecoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_blokecoin_tx()
        self.skip_if_no_blokecoin_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "blokecoin-util-test.json", encoding="utf8") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./blokecoin-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./blokecoin-tx":
