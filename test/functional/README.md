...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/blokecoin/blokecoin/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the BlokecoinTestFramework, place overrides for the
...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `BlokecoinTestFramework` class, which can be used to skip a whole test
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  initialize an empty blokechain and start from the Genesis block, rather than
  load a premined blokechain from cache with the default value of `False`. The
  cached data directories contain a 200-block pre-mined blokechain with the
...
with the blokecoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a blokecoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Blokecoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the BlokecoinTestFramework
...
Taken from the [python-blokecoinrpc repository](https://github.com/jgarzik/python-blokecoinrpc).
...
Test objects for interacting with a blokecoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-blokecoinlib)
