...
class ConfArgsTest(BlokecoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'blokecoin.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "blokecoin.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in blokecoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "blokecoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'blokecoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "blokecoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by blokecoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a blokecoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.blokecoinconf.read_text()
        conf_path = default_datadir / "blokecoin.conf"
        ...
        # (conf_path, not node.blokecoinconf)
    ...
    def test_seed_peers(self):
        ...
        # -fixedseeds in tests
        util.assert_equal(self.nodes[0].getblokechaininfo()['chain'],'regtest')
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a blokecoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "blokecoin.conf" file which is ignored, because a '
        ...
        # the same blokecoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/blokecoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when blokecoin.conf in the default data directory sets another datadir '
                      'and it contains a different blokecoin.conf file that would be ignored')
        ...
        # blokecoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "blokecoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "blokecoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/blokecoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "blokecoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "blokecoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_blokecoin.conf"
