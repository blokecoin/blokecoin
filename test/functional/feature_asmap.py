#!/usr/bin/env python3
# Copyright (c) 2020-2021 The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test asmap config argument for ASN-based IP bucketing.

Verify node behaviour and debug log when launching blokecoind in these cases:

1. `blokecoind` with no -asmap arg, using /16 prefix for IP bucketing

2. `blokecoind -asmap=<absolute path>`, using the unit test skeleton asmap

3. `blokecoind -asmap=<relative path>`, using the unit test skeleton asmap

4. `blokecoind -asmap/-asmap=` with no file specified, using the default asmap

5. `blokecoind -asmap` restart with an addrman containing new and tried entries

6. `blokecoind -asmap` with no file specified and a missing default asmap file

7. `blokecoind -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BlokecoinTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test blokecoind with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_noasmap_arg(self):
        self.log.info('Test blokecoind with -noasmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0, ["-noasmap"])

    def test_asmap_with_absolute_path(self):
        self.log.info('Test blokecoind -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test blokecoind -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test blokecoind {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test blokecoind -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test blokecoind -asmap with missing default map file')
        self.stop_node(0)
        msg = f"Error: Could not find asmap file \"{self.default_asmap}\""
        self.node.assert_start_raises_init_error(extra_args=['-asmap'], expected_msg=msg)

    def test_empty_asmap(self):
        self.log.info('Test blokecoind -asmap with empty map file')
        self.stop_node(0)
    ...
    def test_asmap_health_check(self):
        self.log.info('Test blokecoind -asmap logs ASMap Health Check with basic stats')
        shutil.copyfile(self.asmap_raw, self.default_asmap)
