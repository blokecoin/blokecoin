#!/usr/bin/env python3
# Copyright (c) 2020-present The Blokecoin Core developers
...
class FeatureIndexPruneTest(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 689']):
                pruneheight_new = node.pruneblokechain(400)
        ...
        for i in range(3):
            pruneheight_2 = self.nodes[i].pruneblokechain(1000)
        ...
        for i in range(3):
            pruneheight_3 = self.nodes[i].pruneblokechain(2000)
        ...
        filter_msg = "Error: basic block filter index best block of the index goes beyond pruned data. Please disable the index or reindex (which will download the whole blokechain again)"
        stats_msg = "Error: coinstatsindex best block of the index goes beyond pruned data. Please disable the index or reindex (which will download the whole blokechain again)"
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 2489']):
                pruneheight_new = node.pruneblokechain(2500)
