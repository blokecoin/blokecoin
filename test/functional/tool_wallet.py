#!/usr/bin/env python3
# Copyright (c) 2018-2022 The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test blokecoin-wallet."""
...
class ToolWalletTest(BlokecoinTestFramework):
    ...
    def blokecoin_wallet_process(self, *args):
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.blokecoin_wallet_process(*args)
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your Blokecoin, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `blokecoin-wallet help` raises an error. Use `blokecoin-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `blokecoin-wallet -help` for valid methods.')
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of blokecoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of blokecoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
