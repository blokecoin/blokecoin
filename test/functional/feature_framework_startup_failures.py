#!/usr/bin/env python3
# Copyright (c) 2025-present The Blokecoin Core developers
...
without the parent process' BlokecoinTestFramework also failing.
"""
...
class FeatureFrameworkStartupFailures(BlokecoinTestFramework):
    ...
    def _verify_startup_failure(self, test, internal_args, expected_exception):
        ...
        try:
            ...
            print("Unexpected child process timeout!\n"
                  ...
                  "meaning dangling blokecoind processes are to be expected.\n"
    ...
    def run_test(self):
        ...
        self.log.info("Verifying inability to connect to blokecoind's RPC interface due to wrong port results in one exception containing at least one OSError.")
        self._verify_startup_failure(
            TestWrongRpcPortStartupFailure, [f"--internal_node_start_duration={node_start_duration}"],
            r"AssertionError: \[node 0\] Unable to connect to blokecoind after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest: '[\w ]+'/\w+\([^)]+\)\)"
        )

        self.log.info("Verifying startup failure due to invalid arg results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorStartupFailure, [],
            r"FailedToStartError: \[node 0\] blokecoind exited with status 1 during initialization\. Error: Error parsing command line arguments: Invalid parameter -nonexistentarg"
        )
...
class TestWrongRpcPortStartupFailure(InternalTestMixin, BlokecoinTestFramework):
    def add_options(self, parser):
...
class TestInitErrorStartupFailure(InternalTestMixin, BlokecoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestStartStopStartupFailure(InternalTestMixin, BlokecoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestSuccess(InternalTestMixin, BlokecoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
