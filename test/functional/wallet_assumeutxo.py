...
class AssumeutxoTest(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        for n in self.nodes:
            assert_equal(n.getblokechaininfo()[
        ...
        time = n1.getblokechaininfo()['time']
        timestamp = 0
        ...
        assert_raises_rpc_error(-1, "Failed to rescan unavailable blocks likely due to an in-progress assumeutxo background sync. Check logs or getchainstates RPC for assumeutxo background sync progress and try again later.", w1.rescanblokechain, 100)
