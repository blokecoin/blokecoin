...
# TestP2PConn: A peer we use to send messages to blokecoind, and store responses.
...
class CompactBlocksTest(BlokecoinTestFramework):
    ...
    # This test actually causes blokecoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that blokecoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for blokecoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # blokecoind will not send blocktxn responses for blocks whose height is
