#!/usr/bin/env python3
# Copyright (c) 2019-2022 The Blokecoin Core developers
...
class ImportDescriptorsTest(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
            ...
            try:
                ...
                assert e.error["code"] == -4 and "Error: the wallet is currently being used to rescan the blokechain for related transactions. Please call `abortrescan` before locking the wallet." in e.error["message"]

            try:
                self.nodes[0].cli("-rpcwallet=encrypted_wallet").walletpassphrasechange("passphrase", "newpassphrase")
            except JSONRPCException as e:
                assert e.error["code"] == -4 and "Error: the wallet is currently being used to rescan the blokechain for related transactions. Please call `abortrescan` before changing the passphrase." in e.error["message"]
