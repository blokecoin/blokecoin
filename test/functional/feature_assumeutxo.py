#!/usr/bin/env python3
# Copyright (c) 2021-present The Blokecoin Core developers
...
to a hash that has been compiled into blokecoind.
...
class AssumeutxoTest(BlokecoinTestFramework):
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test blokecoind should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test blokecoind should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test blokecoind should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
    ...
    def run_test(self):
        ...
        assert_equal(n0.getblockcount(), SNAPSHOT_BASE_HEIGHT)
        assert_equal(n0.getblokechaininfo()["blocks"], SNAPSHOT_BASE_HEIGHT)
        ...
        for n in self.nodes:
            assert_equal(n.getblokechaininfo()["headers"], SNAPSHOT_BASE_HEIGHT)
        ...
        assert_equal(n0.getblokechaininfo()["blocks"], FINAL_HEIGHT)
        ...
        assert_equal(n1.getblokechaininfo()["blocks"], SNAPSHOT_BASE_HEIGHT)
        ...
        for i in (0, 1):
            ...
            assert_equal(n.getblokechaininfo()["blocks"], FINAL_HEIGHT)
        ...
        assert_equal(n2.getblokechaininfo()["blocks"], FINAL_HEIGHT)
