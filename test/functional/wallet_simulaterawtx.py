#!/usr/bin/env python3
# Copyright (c) 2021-2022 The Blokecoin Core developers
...
class SimulateTxTest(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        blokecoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 btc decrease, w2 sees + 5 btc
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + blokecoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - blokecoin_fee + 4.9999       -5 - blokecoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - blokecoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - blokecoin_fee)
        ...
        tx2 = funding["hex"]
        blokecoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + blokecoin_fee2))
