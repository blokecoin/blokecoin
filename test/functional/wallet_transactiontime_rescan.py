...
class TransactionTimeRescanTest(BlokecoinTestFramework):
    ...
    def run_test(self):
        ...
        restorewo_wallet.rescanblokechain(initial_mine + 350)
        self.log.info('Rescan all history')
        restorewo_wallet.rescanblokechain()
        ...
        self.log.info('Test handling of invalid parameters for rescanblokechain')
        assert_raises_rpc_error(-8, "Invalid start_height", restorewo_wallet.rescanblokechain, -1, 10)
        assert_raises_rpc_error(-8, "Invalid stop_height", restorewo_wallet.rescanblokechain, 1, -1)
        assert_raises_rpc_error(-8, "stop_height must be greater than start_height", restorewo_wallet.rescanblokechain, 20, 10)

        self.log.info("Test `rescanblokechain` fails when wallet is encrypted and locked")
        usernode.createwallet(wallet_name="enc_wallet", passphrase="passphrase")
        enc_wallet = usernode.get_wallet_rpc("enc_wallet")
        assert_raises_rpc_error(-13, "Error: Please enter the wallet passphrase with walletpassphrase first.", enc_wallet.rescanblokechain)
