#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Blokecoin Core developers
...
class Binaries:
    """Helper class to provide information about blokecoin binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke blokecoind"
        return self._argv("node", self.paths.blokecoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke blokecoin-cli"
        # Add -nonamed because "blokecoin rpc" enables -named by default, but blokecoin-cli doesn't
        return self._argv("rpc", self.paths.blokecoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke blokecoin-tx"
        return self._argv("tx", self.paths.blokecointx)

    def util_argv(self):
        "Return argv array that should be used to invoke blokecoin-util"
        return self._argv("util", self.paths.blokecoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke blokecoin-wallet"
        return self._argv("wallet", self.paths.blokecoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke blokecoin-chainstate"
        return self._argv("chainstate", self.paths.blokecoinchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the blokecoin wrapper executable (if BITCOIN_CMD is set or
        need_ipc is True), or the direct binary path (blokecoind, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.blokecoin_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the blokecoin
            # wrapper binary and append -m so it calls multiprocess binaries.
            blokecoin_cmd = self.paths.blokecoin_cmd or [self.paths.blokecoin_bin]
            return blokecoin_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
def get_binary_paths(config):
    ...
    binaries = {
        "blokecoin": "BITCOIN_BIN",
        "blokecoind": "BITCOIND",
        "blokecoin-cli": "BITCOINCLI",
        "blokecoin-util": "BITCOINUTIL",
        "blokecoin-tx": "BITCOINTX",
        "blokecoin-chainstate": "BITCOINCHAINSTATE",
        "blokecoin-wallet": "BITCOINWALLET",
    }
    # Set paths to blokecoin core binaries allowing overrides with environment
    # variables.
    ...
    # BITCOIN_CMD environment variable can be specified to invoke blokecoin
    # wrapper binary instead of other executables.
    paths.blokecoin_cmd = shlex.split(os.getenv("BITCOIN_CMD", "")) or None
    return paths
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `BlokecoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/blokecoin/blokecoin/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "blokecoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Blokecoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Blokecoin"
        else:
            datadir = temp_dir / ".blokecoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "blokecoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "blokecoin.conf")):
        with open(os.path.join(datadir, "blokecoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
