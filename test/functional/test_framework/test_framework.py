#!/usr/bin/env python3
# Copyright (c) 2014-present The Blokecoin Core developers
...
TMPDIR_PREFIX = "blokecoin_func_test_"
...
class BlokecoinTestMetaClass(type):
    """Metaclass for BlokecoinTestFramework.

    Ensures that any attempt to register a subclass of `BlokecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BlokecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BlokecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BlokecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BlokecoinTestFramework(metaclass=BlokecoinTestMetaClass):
    """Base class for a blokecoin test script.

    Individual blokecoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave blokecoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use blokecoin-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BlokecoinRPC")
    ...
    def setup_chain(self):
        """Override this method to customize blokechain setup"""
    ...
    def setup_nodes(self):
        ...
        if not self.setup_clean_chain:
            for n in self.nodes:
                assert_equal(n.getblokechaininfo()["blocks"], 199)
            ...
            for n in self.nodes:
                n.submitblock(block)
                chain_info = n.getblokechaininfo()
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a blokecoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple blokecoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the blokecoind
    ...
    def _start_logging(self):
        ...
        # Format logs the same as blokecoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        """Initialize a pre-mined blokechain for use by the test.
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            assert_equal(cache_node.getblokechaininfo()["blocks"], 199)
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in blokecoin.conf

    def _initialize_chain_clean(self):
        """Initialize empty blokechain for use by the test.

        Create an empty blokechain and num_nodes wallets.
    ...
    def skip_if_no_blokecoind_tracepoints(self):
        """Skip the running test if blokecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("blokecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_blokecoind_zmq(self):
        """Skip the running test if blokecoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("blokecoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if blokecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("blokecoin-wallet has not been compiled")

    def skip_if_no_blokecoin_tx(self):
        """Skip the running test if blokecoin-tx has not been compiled."""
        if not self.is_blokecoin_tx_compiled():
            raise SkipTest("blokecoin-tx has not been compiled")

    def skip_if_no_blokecoin_util(self):
        """Skip the running test if blokecoin-util has not been compiled."""
        if not self.is_blokecoin_util_compiled():
            raise SkipTest("blokecoin-util has not been compiled")

    def skip_if_no_blokecoin_chainstate(self):
        """Skip the running test if blokecoin-chainstate has not been compiled."""
        if not self.is_blokecoin_chainstate_compiled():
            raise SkipTest("blokecoin-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if blokecoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("blokecoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether blokecoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether blokecoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_blokecoin_tx_compiled(self):
        """Checks whether blokecoin-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_blokecoin_util_compiled(self):
        """Checks whether blokecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_blokecoin_chainstate_compiled(self):
        """Checks whether blokecoin-chainstate was compiled."""
