#!/usr/bin/env python3
# Copyright (c) 2017-present The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for blokecoind node under test"""
...
# 'blokecoin' command, it may internally insert more args, which must be
...
BITCOIN_PID_FILENAME_DEFAULT = "blokecoind.pid"
...
class TestNode():
    """A class for representing a blokecoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        self.blokecoinconf = self.datadir_path / "blokecoin.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the blokecoin.conf file.
        # This means that starting a blokecoind using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    def __del__(self):
        # Ensure that we don't leave any blokecoind processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then blokecoind would bind P2P ports on
        ...
        # blokecoin.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time blokecoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by blokecoind, and
        ...
        self.running = True
        self.log.debug("blokecoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the blokecoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'blokecoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; blokecoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to blokecoind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; blokecoind is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into blokecoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to blokecoind
        expected_msg: regex that stderr should match when blokecoind fails

        Will throw if blokecoind starts without an error.
        Will throw if an expected_msg is provided and it does not match blokecoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'blokecoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'blokecoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to blokecoin-cli for an individual node"""
    def __init__(self, binaries, datadir, rpc_timeout):
        ...
        self.input = None
        self.log = logging.getLogger('TestFramework.blokecoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with blokecoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run blokecoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running blokecoin-cli {}".format(p_args[2:]))
