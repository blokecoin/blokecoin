...
# Copyright (c) 2010-present The Blokecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test objects for interacting with a blokecoind node over the p2p protocol.

The P2PInterface objects interact with the blokecoind nodes under test using the
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor, supports_v2_p2p):
        ...
        logger.debug('Connecting to Blokecoin Node: %s:%d' % (self.dstaddr, self.dstport))
    ...
    def peer_accept_connection(self, connect_id, connect_cb=lambda: None, *, net, timeout_factor, supports_v2_p2p, reconnect):
        ...
        logger.debug('Listening for Blokecoin Node with id: {}'.format(connect_id))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a Blokecoin node.
