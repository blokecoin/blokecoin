...
This tutorial will describe how to use two instances of Blokecoin Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed Blokecoin Transactions](https://github.com/blokecoin/blokecoin/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
## Overview
In this tutorial we have two hosts, both running Blokecoin v25.0

* `offline` host which is disconnected from all networks (internet, Tor, wifi, bluetooth etc.) and does not have, or need, a copy of the blokechain.
* `online` host which is a regular online node with a synced blokechain.
...
```sh
[offline]$ ./build/bin/blokecoin-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/bin/blokecoin-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -named createwallet \
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/bin/blokecoin-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/blokecoin-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/blokecoin-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/blokecoin-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/blokecoin-cli -signet -rpcwallet="watch_only_wallet" listtransactions
