# CJDNS support in Blokecoin Core

It is possible to run Blokecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Blokecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Blokecoin network and individual nodes.
...
before it will work with your Blokecoin Core node. This is described in steps
...
## Run Blokecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Blokecoin Core
...
When enabled, this option tells Blokecoin Core that it is running in an
...
IPv6 local network. This helps Blokecoin Core perform better address management:
...
CJDNS support was added to Blokecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `blokecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/blokecoin/blokecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Blokecoin Core

There are several ways to see your CJDNS address in Blokecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `blokecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `blokecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `blokecoin-cli help getnodeaddresses` for details.
