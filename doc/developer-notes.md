...
Blokecoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while blokecoind is running with the `logging`
RPC.  For instance, launching blokecoind with `-debug` or `-debug=1` will turn on
...
with "play blokecoins" on a test network.
...
Blokecoin Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at blokecoind startup or
`blokecoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `blokecoin-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/blokecoin/blokecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_blokecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_blokecoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/blokecoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_blokecoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_blokecoin \
    --object=build/bin/blokecoind \
    ...
    --project-title="Blokecoin Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Blokecoin Core Fuzz Coverage Report"
```
...
To profile a running blokecoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep blokecoind` -- sleep 60
```
...
Blokecoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`blokecoind`)](https://doxygen.blokecoincore.org/blokecoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `blokecoind.cpp`. Responsible for starting up and
  shutting down the application.

- [Init load (`b-initload`)](https://doxygen.blokecoincore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
...
- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.blokecoincore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.blokecoincore.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.blokecoincore.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.blokecoincore.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.blokecoincore.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.blokecoincore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.blokecoincore.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.blokecoincore.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.blokecoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.blokecoincore.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.blokecoincore.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.blokecoincore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
    : Listens for and accepts incoming I2P connections through the I2P SAM proxy.

# Development guidelines

A few non-style-related recommendations for developers, as well as points to
pay attention to for reviewers of Blokecoin Core code.

## General Blokecoin Core
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Blokecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Blokecoin Core, in which case
...
a Blokecoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Blokecoin Core.

To fully verify or update a subtree, add it as a remote:

```sh
git remote add libmultiprocess https://github.com/blokecoin-core/libmultiprocess.git
```
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Blokecoin Core subtree to avoid delay.
...
file descriptors, it will cause problems with Blokecoin's `select()` loop, because
...
compatibility between nodes. This happened in Blokecoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Blokecoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/blokecoin/blokecoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/blokecoin/blokecoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/blokecoin/blokecoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blokecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blokecoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  `getblokechaininfo`'s state immediately prior to the call's execution. Wallet
  RPCs whose behavior does *not* depend on the current chainstate may omit this
  call.

  - *Rationale*: In previous versions of Blokecoin Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/blokecoin/blokecoin/pull/14635).
