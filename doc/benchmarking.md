...
Blokecoin Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_blokecoin`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_blokecoin

After compiling blokecoin-core, the benchmarks can be run with:

    build/bin/bench_blokecoin
...
To monitor Blokecoin Core performance more in depth (like reindex or IBD): https://github.com/blokecoin-dev-tools/benchcoin
