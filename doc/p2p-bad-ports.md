When Blokecoin Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Blokecoin node is listening,
or one where a service is listening that is not related to the Blokecoin network.
As a result, this service may occasionally get connection attempts from Blokecoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Blokecoin Core,
trying to connect because it thinks there is a Blokecoin node on that
...
Below is a list of "bad" ports which Blokecoin Core avoids when choosing a peer to
...
[pull/23306](https://github.com/blokecoin/blokecoin/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/blokecoin/blokecoin/pull/23542)
