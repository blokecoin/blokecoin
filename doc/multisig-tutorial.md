...
Currently, it is possible to create a multisig wallet using Blokecoin Core only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/blokecoin/blokecoin/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the blokecoin node on the signet network.

```bash
./build/bin/blokecoind -signet -daemon
```

This tutorial also uses the default WPKH derivation path to get the xpubs and does not conform to [BIP 45](https://github.com/blokecoin/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/blokecoin/bips/blob/master/bip-0087.mediawiki).

At the time of writing, there is no way to extract a specific path from wallets in Blokecoin Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/blokecoin-cli -signet createwallet "participant_${n}"
done
```

`blokecoin rpc` can also be substituted for `blokecoin-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Blokecoin Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Blokecoin Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
As previously mentioned, this step extracts the `m/84'/1'/0'` account instead of the path defined in [BIP 45](https://github.com/blokecoin/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/blokecoin/bips/blob/master/bip-0087.mediawiki), since there is no way to extract a specific path in Blokecoin Core at the time of writing.
...
external_desc_sum=$(./build/bin/blokecoin-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/blokecoin-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
`external_desc` and `internal_desc` specify the output type (`wsh`, in this case) and the xpubs involved. They also use BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/blokecoin/blokecoin/issues/21199#issuecomment-780772418)].

Note that at least two descriptors are usually used, one for internal derivation paths and one for external ones. There are discussions about eliminating this redundancy, as can be seen in the issue [#17190](https://github.com/blokecoin/blokecoin/issues/17190).
...
* `timestamp`: Sets the time from which to start rescanning the blokechain for the descriptor, in UNIX epoch time.

Documentation for these and other parameters can be found by typing `./build/bin/blokecoin-cli help importdescriptors`.
...
```bash
./build/bin/blokecoin-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/blokecoin-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/blokecoin-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/blokecoin-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/blokecoin-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/blokecoin-cli -a $receiving_address
```
...
```bash
./build/bin/blokecoin-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Blokecoin Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Blokecoin transaction and the signatures necessary to complete it. [[source](https://blokecoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/blokecoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid blokecoin address.
...
```bash
balance=$(./build/bin/blokecoin-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/blokecoin-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://blokecointalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./build/bin/blokecoin-cli -signet decodepsbt $funded_psbt

./build/bin/blokecoin-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/blokecoin-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/blokecoin-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/blokecoin-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/blokecoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/blokecoin-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
