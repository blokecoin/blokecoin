...
Blokecoin Core version 0.21.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/blokecoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Blokecoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Blokecoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/blokecoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/blokecoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/blokecoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/blokecoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/blokecoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `blokecoin-cli -netinfo` command provides a network peer connections
...
- A new `blokecoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `blokecoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `blokecoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Blokecoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `blokecoin-wallet` tool which performs the salvage
...
Blokecoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`blokecoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Blokecoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `blokecoin-cli createwallet` or `blokecoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/blokecoin/blokecoin/issues).
...
Users who already have a Blokecoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Blokecoin Core.
...
The change to using descriptors changes the default derivation paths used by Blokecoin Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Blokecoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `blokecoin.conf`
...
- #19284 Add seed.blokecoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in blokecoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add blokecoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in blokecoin-cli (laanwj)
...
### Build system
- #18504 Drop blokecoin-tx and blokecoin-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for blokecoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Blokecoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreblokecoin)
...
- #18817 Document differences in blokecoind and blokecoin-qt locale handling (practicalswift)
