...
Blokecoin Core version 24.0.1 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-24.0.1/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Blokecoin
...
This version of Blokecoin Core adds a new `mempoolfullrbf` configuration
...
Some Blokecoin services today expect that the first version of an
...
The Blokecoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Blokecoin Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Blokecoin Core are
...
[Blokecoin Core 0.12.0]: https://blokecoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/blokecoin/bips/blob/master/bip-0125.mediawiki
...
- The `-deprecatedrpc=softforks` configuration option has been removed.  The
  RPC `getblokechaininfo` no longer returns the `softforks` field, which was
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Blokecoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://blokecoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Blokecoin Core wallet, change output
...
[documentation](https://github.com/blokecoin/blokecoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the blokecoin GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to blokecoind, instead of being
ignored. (#15936, gui#602)

- Also, the interaction between GUI settings and `blokecoin.conf` settings is
simplified. Settings from `blokecoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`blokecoin.conf` values. (#15936)
