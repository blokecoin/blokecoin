Blokecoin Core version 0.18.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-0.18.0/>
...
`/Applications/Blokecoin-Qt` (on Mac) or `blokecoind`/`blokecoin-qt` (on
Linux).
...
redownloading the blokechain is not supported.  However, as usual, old
...
recommended to use Blokecoin Core on unsupported systems.

Blokecoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Blokecoin Core does not yet change appearance when
...
`contrib/init/blokecoind.service`, it has been changed to use
`/var/lib/blokecoind` as the data directory instead of
`~blokecoin/.blokecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/blokecoind` will
exist has enough space (check using `df -h /var/lib/blokecoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/blokecoin/blokecoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://blokecoincore.org/en/contact/) the Blokecoin Core
...
The systemd init file (`contrib/init/blokecoind.service`) has been changed
to use `/var/lib/blokecoind` as the data directory instead of
`~blokecoin/.blokecoin`. This change makes Blokecoin Core more consistent with
...
`datadir` option in `/etc/blokecoin/blokecoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/blokecoin/blokecoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/blokecoin/blokecoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/blokecoin/blokecoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/blokecoin/blokecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/blokecoin/blokecoin/blob/master/doc/blokecoin-conf.md)
  about the `blokecoin.conf` file describes how to use it to configure
  Blokecoin Core.

- A new document introduces Blokecoin Core's BIP174 [Partially-Signed
  Blokecoin Transactions
  (PSBT)](https://github.com/blokecoin/blokecoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/blokecoin/blokecoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Blokecoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Blokecoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/blokecoin/blokecoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/blokecoin/blokecoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Blokecoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Blokecoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/blokecoin/blokecoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/blokecoin/blokecoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart blokecoind with the `-deprecatedrpc=generate`
...
- A new `/rest/blockhashbyheight/` endpoint is added for fetching the
  hash of the block in the current best blokechain based on its height
...
- A new `blokecoin-wallet` tool is now distributed alongside Blokecoin
...
This section describes planned changes to Blokecoin Core that may affect
other Blokecoin software and services.

- Since version 0.16.0, Blokecoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Blokecoin Core 0.20 (expected about
  a year after 0.18), Blokecoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Blokecoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Blokecoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Blokecoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Blokecoin Core
...
- A sub-project of Blokecoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Blokecoin Core.  See their [project
  page](https://github.com/blokecoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Blokecoin Core's own implementation, although entropy
  gathered by Blokecoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Blokecoin Core a
...
- On macOS, Blokecoin Core now opts out of application CPU throttling
  ("app nap") during initial blokechain download, when catching up from
...
- #12490 Remove deprecated wallet rpc features from `blokecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libblokecoin_server.a` (jnewbery)
...
- #15279 Clarify rescanblokechain doc (MarcoFalke)
- #15292 Remove `boost::optional`-related false positive -Wmaybe-uninitialized warnings on GCC compiler (hebasto)
- #13926 [Tools] blokecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like blokecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `blokecoin-qt -version` output formatting (hebasto)
...
- #11625 Add BlokecoinApplication & RPCConsole tests (ryanofsky)
...
- #15183 Fix `m_assumed_blokechain_size` variable value (marcoagner)
...
- #14204 Move `interfaces/*` to `libblokecoin_server` (laanwj)
...
- #15154 Configure: blokecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/blokecoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Blokecoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libblokecoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in blokecoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Blokecoin Core (Christewart)
...
- #12246 Bugfix: Only run blokecoin-tx tests when blokecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_blokecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_blokecoin` (MarcoFalke)
...
- #14186 blokecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_blokecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update blokecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/blokecoin-conf.md (hebasto)
...
- #15354 Add missing `blokecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct blokecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support blokecoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/blokecoin/).
