Blokecoin Core version 0.14.0 is now available from:

  <https://blokecoin.org/bin/blokecoin-core-0.14.0/>
...
Blokecoin Core has supported automatically pruning the blokechain since 0.11. Pruning
the blokechain allows for significant storage space savings as the vast majority of
the downloaded data can be discarded after processing so very little of it remains
on the disk.

Manual block pruning can now be enabled by setting `-prune=1`. Once that is set,
the RPC command `pruneblokechain` can be used to prune the blokechain up to the
...
`"blocks"`	   | `getblokechaininfo()["blocks"]`
`"timeoffset"`	   | `getnetworkinfo()["timeoffset"]`
`"connections"`	   | `getnetworkinfo()["connections"]`
`"proxy"`	   | `getnetworkinfo()["networks"][0]["proxy"]`
`"difficulty"`	   | `getblokechaininfo()["difficulty"]`
`"testnet"`	   | `getblokechaininfo()["chain"] == "test"`
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/blokecoin/blokecoin/blob/master/doc/zmq.md) for
...
When Blokecoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `blokecoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`blokecoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/blokecoin-cli -named help command="help"
    src/blokecoin-cli -named getblockhash height=0
    src/blokecoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/blokecoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://blokecoin.org/en/alert/2016-11-01-alert-retirement) in Blokecoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Blokecoin Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/blokecoin/blokecoin/pull/8753)
...
- #8722 `bb843ad` blokecoin-cli: More detailed error reporting (laanwj)
...
- #7948 `5d2c8e5` Augment getblokechaininfo bip9\_softforks data (mruddy)
...
- #9518 `a65ced1` Return height of last block pruned by pruneblokechain RPC (ryanofsky)
...
- #8830 `ef0801b` test: Add option to run blokecoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to blokecoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to blokecoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and blokecoin-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_blokecoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change blokecoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` blokecoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow blokecoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` blokecoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to blokecoin-tx (jnewbery)
