...
Blokecoin Core version v30.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Blokecoin-Qt` (on macOS)
...
Blokecoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Blokecoin
...
New `blokecoin` command
---------------------

- A new `blokecoin` command line tool has been added to make features more discoverable
  and convenient to use. The `blokecoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `blokecoin node` is a synonym for
  `blokecoind`, `blokecoin gui` is a synonym for `blokecoin-qt`, and `blokecoin rpc` is a synonym
  for `blokecoin-cli -named`. Other commands and options can be listed with `blokecoin help`.
  The new `blokecoin` command is an alternative to calling other commands directly, but it
...
- The new `blokecoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `blokecoin -m node -ipcbind=unix` it will
  ...
  (`blokecoin-node` instead of `blokecoind`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/blokecoin/blokecoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/blokecoin/blokecoin/blob/master/doc/multiprocess.md)),
...
- The `test_blokecoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `blokecoin` command
  as `blokecoin test`. The `libexec/` directory also contains new `blokecoin-node` and
  `blokecoin-gui` binaries which support IPC features and are called through the
  `blokecoin` tool. In source builds only, `test_blokecoin-qt`, `bench_blokecoin`, and
  `blokecoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `blokecoin` command. See `blokecoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Blokecoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Blokecoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the blokecoin-wallet tool,
...
[Transifex](https://explore.transifex.com/blokecoin/blokecoin/).
