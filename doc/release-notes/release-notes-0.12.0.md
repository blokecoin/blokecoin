Blokecoin Core version 0.12.0 is now available from:

  <https://blokecoin.org/bin/blokecoin-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Blokecoin Core or other software:
...
pre-0.12 versions of Blokecoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Blokecoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Blokecoin transactions now use validation using
[libsecp256k1](https://github.com/blokecoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/blokecoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Blokecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Blokecoin Core was to raise the default minimum
relay fee.

Blokecoin Core 0.12 will have a strict maximum size on the mempool. The
...
Blokecoin Core 0.12 also introduces new default policy limits on the length and
...
Blokecoin Core 0.12 nodes. Blokecoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/blokecoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Blokecoin Core 0.12 does not yet have support for
...
This allows running blokecoind without having to do any manual configuration.
...
Blokecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Blokecoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Blokecoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Blokecoin Core 0.13, and it is at this time undecided whether the
...
Blokecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Blokecoin Core automatically creates a hidden service to listen on, without
manual configuration. Blokecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Blokecoin Core is listening, and
...
Blokecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Blokecoin
Core to use floating fees. By default, Blokecoin Core will use floating
fees.
...
At all times, Blokecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, Blokecoin Core will never create transactions paying less than
...
`blokecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.

A value of 0 disables pruning. The minimal value above 0 is 550. Your
wallet is as secure with high values as it is with low ones. Higher
values merely ensure that your node will not shut down upon blokechain
...
notes of v0.11.0](https://github.com/blokecoin/blokecoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/blokecoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/blokecoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /blokecoinrpc>
        ...
        # optional bypass blokecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< blokecoinrpc:<password>
    </Location>
...
Restarting blokecoind will no longer clear out the list of banned peers; instead
...
- #6353 `6bcb0a2` Show softfork status in getblokechaininfo (Wladimir J. van der Laan)
...
- #7011 `31de241` Add mediantime to getblokechaininfo (Peter Todd)
...
- #6214 `87406aa` [OSX] revert renaming of Blokecoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new blokecoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include blokecoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix blokecoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of blokecoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [blokecoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libblokecoinconsensus (Braydon Fuller)
