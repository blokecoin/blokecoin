Blokecoin Core version 0.9.2.1 is now available from:

  https://blokecoin.org/bin/0.9.2.1/
...
0.9.2.1 your blokechain files will be re-indexed, which will take anywhere from 
...
- Add `getwalletinfo`, `getblokechaininfo` and `getnetworkinfo` calls (will replace hodge-podge `getinfo` at some point)
...
- Fix "Start blokecoin on system login"
