Blokecoin Core version 0.19.0.1 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-0.19.0.1/>
...
Blokecoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Blokecoin Core on unsupported systems.
...
to open the Blokecoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Blokecoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/blokecoin/blokecoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Blokecoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  the entire blokechain.  Filters will be created in the background and
  currently use about 4 GiB of space.  Note: this version of Blokecoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `blokecoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Blokecoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Blokecoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Blokecoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Blokecoin Core,
  ...
  This prevents Blokecoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Blokecoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Blokecoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `blokecoin-cli -getinfo` has been renamed to
...
- `getblokechaininfo` no longer returns a `bip9_softforks` object.
  Instead, information has been moved into the `softforks` object and
  an additional `type` field describes how Blokecoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Blokecoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Blokecoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Blokecoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  this flag on an existing wallet, rescanning the blokechain is required
  to correctly mark previously used destinations.  Together with "avoid
  partial spends" (added in Blokecoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Blokecoin AEAD (jonasschnelli)
...
- #15639 blokecoin-wallet tool: Drop libblokecoin_server.a dependency (ryanofsky)
...
- #16817 Fix casing in getblokechaininfo to be inline with other fields (dangershony)
...
- #16366 init: Use InitError for all errors in blokecoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "blokecoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of blokecoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from blokecoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_blokecoin to auto generated project files (sipsorcery)
...
- #15773 Add BlokecoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_blokecoin output (practicalswift)
...
- #15896 feature_filelock, interface_blokecoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Blokecoin Core (luke-jr)
...
- #16329 Add tests for getblokechaininfo.softforks (MarcoFalke)
...
- #16277 Suppress output in test_blokecoin for expected errors (gertjaap)
...
- #16489 log: harmonize blokecoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (blokecoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example blokecoin.conf (fanquake)
...
- #16080 build/doc: update blokecoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in blokecoin.conf (fanquake)
...
- #16621 add default blokecoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the blokecoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, blokecoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/blokecoind.cpp (ch4ot1c)
...
- #15459 add how to calculate blokechain and chainstate size variables to release process (marcoagner)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/blokecoin/blokecoin/).
