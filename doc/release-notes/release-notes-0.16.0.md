Blokecoin Core version 0.16.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-0.16.0/>
...
directly from 0.7.x and earlier without re-downloading the blokechain is not supported.
...
Blokecoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
blokecoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Blokecoin Core is now 4.8. No effort will be
...
### New `rescanblokechain` RPC

A new RPC `rescanblokechain` has been added to manually invoke a blokechain rescan.
The RPC supports start and end-height arguments for the rescan, and can be used in a
multiwallet environment to rescan the blokechain at runtime.
...
- The RPC `getblokechaininfo` now includes an `errors` field.
...
- Blokechain rescans now no longer lock the wallet for the entire rescan process, so other RPCs can now be used at the same time (although results of balances / transactions may be incorrect or incomplete until the rescan is complete).
- The `logging` RPC has now been made public rather than hidden.
- An `initialblockdownload` boolean has been added to the `getblokechaininfo` RPC to indicate whether the node is currently in IBD or not.
- `minrelaytxfee` is now included in the output of `getmempoolinfo`

Other changed command-line options
----------------------------------
- `-debuglogfile=<file>` can be used to specify an alternative debug logging file.
- blokecoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- blokecoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest blokecoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.blokecoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for blokecoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to blokecoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #10858 `9a8e916` Add "errors" field to getblokechaininfo and unify "errors" field in get*info RPCs (achow101)
- #11021 `90926db` Fix getchaintxstats() (AkioNak)
- #11367 `3a93270` getblokechaininfo: Add disk_size, prune_target_size (esotericnonsense)
...
- #11258 `033c786` Add initialblockdownload to getblokechaininfo (jnewbery)
...
- #10874 `9e38d35` getblokechaininfo: Loop through the bip9 soft fork deployments instead of hard coding (achow101)
...
- #7061 `8c2de82` Add RPC call "rescanblokechain <startheight> <stopheight>" (jonasschnelli)
...
- #11621 `1f7695b` [build] Add temp_blokecoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` blokecoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.blokecoin doesn't exist before build (MeshCollider)
...
- #11370 `2d85899` Add getblokechaininfo functional test (promag)
...
- #11748 `20166f8` Adding unit tests for GetDifficulty in blokechain.cpp (merehap)
...
- #11143 `3aa60b7` Fix include path for blokecoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in blokecoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop blokecoind (OmeGak)
...
- #10529 `331352f` Improve blokecoind systemd service file (Flowdalic)
