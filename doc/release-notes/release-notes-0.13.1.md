Blokecoin Core version 0.13.1 is now available from:

  <https://blokecoin.org/bin/blokecoin-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a blokecoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Blokecoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/blokecoin/blokecoin/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Blokecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Blokecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Blokecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of blokecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of blokecoins being spent is exactly
...
- **Increased security for multisig:** Blokecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Blokecoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Blokecoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Blokecoin's current ledger (called the UTXO set),
  making the node vulnerable to falling out of consensus with other nodes.
  Although the problems with Nakamoto's method can't be fixed in a soft fork,
  Segwit accomplishes something similar to his original proposal: it makes it
  possible for a node to optionally skip downloading some blokechain data
  (specifically, the segregated witnesses) while still ensuring that the node
  can build an accurate copy of the UTXO set for the block chain with the most
  proof of work.  Segwit enables this capability at the consensus layer, but
  note that Blokecoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Blokecoin users to individually opt-in to almost any change in the Blokecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Blokecoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Blokecoin community members are studying several other improvements
...
[Segwit FAQ]: https://blokecoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://blokecoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/blokecoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/blokecoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/blokecoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/blokecoin/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://blokecoincore.org/en/2016/06/08/version-bits-miners-faq/
...
Since Blokecoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/blokecoin/bips/blob/master/bip-0147.mediawiki
...
- `blokecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `blokecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in blokecoinconsensus.h (jl2012)
...
- #8884 `b987348` getblokechaininfo help: pruneheight is the lowest, not highest, block (luke-jr)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.blokecoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libblokecoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_blokecoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add blokecoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` blokecoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_blokecoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name blokecoin.qrc  (blokecoinsSG)
...
- #8817 `bcf3806` update blokecoin-tx to output witness data (jnewbery)
