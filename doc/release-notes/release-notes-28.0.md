Blokecoin Core version 28.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-28.0/>
...
Running Blokecoin Core binaries on macOS requires self signing.
```
cd /path/to/blokecoin-28.0/bin
xattr -d com.apple.quarantine blokecoin-cli blokecoin-qt blokecoin-tx blokecoin-util blokecoin-wallet blokecoind test_blokecoin
codesign -s - blokecoin-cli blokecoin-qt blokecoin-tx blokecoin-util blokecoin-wallet blokecoind test_blokecoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Blokecoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/blokecoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Blokecoin`
to `C:\Users\Username\AppData\Local\Blokecoin`. Blokecoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/blokecoin/blokecoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libblokecoinconsensus Removal
---------------------------

The libblokecoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Blokecoin Core was listening for P2P connections, either using
...
- Blokecoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/blokecoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblokechaininfo`, `getmininginfo` and
  `getnetworkinfo` now returns all the active node warnings as an array
  of strings, instead of a single warning. The current behaviour
  can be temporarily restored by running Blokecoin Core with the configuration
...
are now required to compile Blokecoin Core. (#29091, #30263)

- The minimum required glibc to run Blokecoin Core is now
...
- A new `-testdatadir` option has been added to `test_blokecoin` to allow specifying the
...
Previous releases of Blokecoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
