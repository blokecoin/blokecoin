...
Blokecoin Core version 25.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-25.0/>
...
- The scanblocks RPC returns the relevant blockhashes from a set of descriptors by
  ...
  the getblockheader and rescanblokechain RPCs to achieve fast wallet rescans. Note
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/blokecoin/blokecoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `blokecoin-cli` tool both take advantage of this, so
  for example:

```sh
blokecoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
blokecoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Blokecoin Core will now return an error at startup. (#25574)
...
before Blokecoin Core has begun its shutdown sequence. (#23395)
...
- Rescans for descriptor wallets are now significantly faster if compact
  ...
  performance of the RPC calls `rescanblokechain`, `importdescriptors`
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching blokecoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/blokecoin/blokecoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
