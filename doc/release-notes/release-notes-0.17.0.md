Blokecoin Core version 0.17.0 is now available from:

  <https://blokecoincore.org/bin/blokecoin-core-0.17.0/>
...
directly from 0.7.x and earlier without redownloading the blokechain is not supported.
...
processing the entire blokechain.

Compatibility
==============

Blokecoin Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `blokecoin.conf` file, not inside included files or from
  ...
  as blokecoin.conf will still include `relative.conf`.

GUI changes
-----------

- Block storage can be limited under Preferences, in the Main tab. Undoing this setting requires downloading the full blokechain again. This mode is incompatible with -txindex and -rescan.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the blokecoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=blokecoin
    test.uacomment=blokecoin-testnet
...
be used in V0.17 by starting blokecoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Blokecoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/blokecoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Blokecoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://blokecointalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Blokecoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Blokecoin transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When blokecoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If blokecoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart blokecoind with
...
- When running blokecoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If blokecoind
...
- #11962 `0a01843` add seed.blokecoin.sprovoost.nl to DNS seeds (Sjors)
...
- #13079 `5778d44` Fix rescanblokechain rpc to properly report progress (Empact)
...
- #13288 `a589f53` Remove the need to include rpc/blokechain.cpp in order to put `GetDifficulty` under test (Empact)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI blokecoin:// (krab)
...
- #13528 `b877c39` Move BlokecoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libblokecoin-util to libblokecoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for blokecoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling blokecoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use BlokecoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to blokecoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use BlokecoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use BlokecoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use BlokecoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for BlokecoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_blokecoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.blokecoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_blokecoin` once (MarcoFalke)
...
- #13924 `09ada21` Simplify comparison in `rpc_blokechain.py` (domob1812)
...
- #12102 `7fb8fb4` Apply hardening measures in blokecoind systemd service file (Flowdalic)
- #12652 `55f490a` blokecoin-cli: Provide a better error message when blokecoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to blokecoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for blokecoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` blokecoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix blokecoin-cli --version (Empact)
...
- #12877 `2b54155` Use blokecoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update blokecoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust blokecoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed blokecoin-cli -help output for help2man (hebasto)
