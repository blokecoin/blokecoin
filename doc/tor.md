...
It is possible to run Blokecoin Core as a Tor onion service, and connect to such services.
...
- Starting with version 22.0, Blokecoin Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by Blokecoin Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via Blokecoin Core

There are several ways to see your local onion address in Blokecoin Core:
...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `blokecoin-cli help getnodeaddresses` for details.

`blokecoin rpc` can also be substituted for `blokecoin-cli`.

## 1. Run Blokecoin Core behind a Tor proxy

The first step is running Blokecoin Core behind a Tor proxy. This will already anonymize all
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    blokecoind -proxy=127.0.0.1:9050

`blokecoin node` or `blokecoin gui` can also be substituted for `blokecoind`.

## 2. Automatically create a Blokecoin Core onion service

Blokecoin Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, Blokecoin Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if Blokecoin Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the blokecoind debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or blokecoind's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running blokecoind must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` blokecoind configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
blokecoind to the same Tor group and setting permissions appropriately.
...
run blokecoind, run this as root:
...
## 3. Manually create a Blokecoin Core onion service

You can also manually configure your node to be reachable from the Tor network.
Add these lines to your `/etc/tor/torrc` (or equivalent config file):

    HiddenServiceDir /var/lib/tor/blokecoin-service/
    HiddenServicePort 8333 127.0.0.1:8334

The directory can be different of course, but virtual port numbers should be equal to
your blokecoind's P2P listen port (8333 by default), and target addresses and ports
should be equal to binding address and port for inbound Tor connections (127.0.0.1:8334 by default).

    -externalip=X   You can tell blokecoin about its publicly reachable addresses using
                    this option, and this can be an onion address. Given the above
                    configuration, you can find your onion address in
                    /var/lib/tor/blokecoin-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    blokecoind -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    blokecoind ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    blokecoind -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover

## 4. Privacy recommendations

- Do not add anything but Blokecoin Core ports to the onion service created in section 3.
  If you run a web service too, create a new onion service for that.
  Otherwise it is trivial to link them, which may reduce privacy. Onion
  services created automatically (as in section 2) always have only one port
  open.
- Operating a node that listens on multiple networks (e.g. IPv4 and Tor) can help
  strengthen the Blokecoin network, as nodes in this configuration (i.e. bridge nodes) increase
