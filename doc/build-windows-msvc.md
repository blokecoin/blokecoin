...
This guide describes how to build blokecoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
### 3. Clone Blokecoin Repository

Clone the Blokecoin Core repository to a directory. All build scripts and commands will run from this directory.
```
...
The Blokecoin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Blokecoin Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to Blokecoin Core.
...
can occur if your local Blokecoin Core repository path contains spaces, you can override the vcpkg install directory
...
To improve the build process performance, one might add the Blokecoin repository directory to the Microsoft Defender Antivirus exclusions.
