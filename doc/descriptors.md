# Support for Output Descriptors in Blokecoin Core

Since Blokecoin Core v0.17, there is support for Output Descriptors. This is a
...
Blokecoin Core v24 extended `wsh()` output descriptor with [Miniscript](https://blokecoin.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25. And since v26 Miniscript expressions can now be used in Taproot descriptors.
...
- [Miniscript](https://blokecoin.sipa.be/miniscript/) expressions in `wsh` (P2WSH) and `tr` (P2TR) functions.
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.blokecoin.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
  - `xpub` encoded extended public key or `xprv` encoded extended private key (as defined in [BIP 32](https://github.com/blokecoin/bips/blob/master/bip-0032.mediawiki)).
...
- P2SH addresses (base58, of the form `3...` for mainnet or `2...` for testnet, defined in [BIP 13](https://github.com/blokecoin/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32 and bech32m, of the form `bc1...` for mainnet or `tb1...` for testnet, defined in [BIP 173](https://github.com/blokecoin/bips/blob/master/bip-0173.mediawiki) and [BIP 350](https://github.com/blokecoin/bips/blob/master/bip-0350.mediawiki)).
...
on Blokecoin's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Blokecoin Core descriptor wallet to serve as your
...
the entire path from the master down. [BIP174](https://github.com/blokecoin/bips/blob/master/bip-0174.mediawiki) standardizes this by
...
existing Blokecoin Core wallets, a convenience function `combo` is
...
All RPCs in Blokecoin Core will include the checksum in their output. Only
