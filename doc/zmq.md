...
The Blokecoin Core daemon can be configured to act as a trusted "border
router", implementing the blokecoin wire protocol and relay, making
consensus decisions, maintaining the local blokechain database,
broadcasting locally generated transactions into the network, and
providing a queryable RPC interface to interact on a polled basis for
requesting blokechain related data. However, there exists only a
...
The ZeroMQ feature in Blokecoin Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ blokecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=unix:/tmp/blokecoind.tx.raw \
...
From the perspective of blokecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into blokecoind directly. Furthermore, no information is
...
using. Blokecoind appends an up-counting sequence number to each
