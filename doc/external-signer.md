# Support for signing transactions outside of Blokecoin Core

Blokecoin Core can be launched with `-signer=<cmd>` where `<cmd>` is an external tool which can sign transactions and perform other functions. For example, it can be used to communicate with a hardware wallet.

## Example usage

The following example is based on the [HWI](https://github.com/blokecoin-core/HWI) tool. Version 2.0 or newer is required. Although this tool is hosted under the Blokecoin Core GitHub organization and maintained by Blokecoin Core developers, it should be used with caution. It is considered experimental and has far less review than Blokecoin Core itself. Be particularly careful when running tools such as these on a computer with private keys on it.

When using a hardware wallet, consult the manufacturer website for (alternative) software they recommend. As long as their software conforms to the standard below, it should be able to work with Blokecoin Core.

Start Blokecoin Core:

```sh
$ blokecoind -signer=../HWI/hwi.py
```

`blokecoin node` can also be substituted for `blokecoind`.

### Device setup

Follow the hardware manufacturers instructions for the initial device setup, as well as their instructions for creating a backup. Alternatively, for some devices, you can use the `setup`, `restore` and `backup` commands provided by [HWI](https://github.com/blokecoin-core/HWI).
...
```
$ blokecoin-cli enumeratesigners
{
  "signers": [
    {
...
```sh
$ blokecoin-cli createwallet "hww" true true "" true true true
```
...
```sh
$ blokecoin-cli -rpcwallet=<wallet> getnewaddress
$ blokecoin-cli -rpcwallet=<wallet> walletdisplayaddress <address>
```
...
Under the hood this uses a [Partially Signed Blokecoin Transaction](psbt.md).

```sh
$ blokecoin-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
```
...
In order to be compatible with Blokecoin Core any signer command should conform to the specification below. This specification is subject to change. Ideally a BIP should propose a standard so that other wallets can also make use of it.

Prerequisite knowledge:
* [Output Descriptors](descriptors.md)
* Partially Signed Blokecoin Transaction ([PSBT](psbt.md))
...
## How Blokecoin Core uses the Signer API
