# I2P support in Blokecoin Core

It is possible to run Blokecoin Core as an
...
## Run Blokecoin Core with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with Blokecoin Core:
...
Once an I2P router with SAM enabled is up and running, use the following Blokecoin
...
```
blokecoind -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `blokecoin-cli help
...
I2P support was added to Blokecoin Core in version 22.0 and there may be fewer I2P
peers than Tor or IP ones. Therefore, using I2P alone without other networks may
make a node more susceptible to [Sybil
attacks](https://en.blokecoin.it/wiki/Weaknesses#Sybil_attack). You can use
`blokecoin-cli -addrinfo` to see the number of I2P addresses known to your node.
...
The first time Blokecoin Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Blokecoin Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Blokecoin Core

There are several ways to see your I2P address in Blokecoin Core if accepting
incoming I2P connections (`-i2pacceptincoming`):
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`
- in the debug log (grep for `AddLocal`; the I2P address ends in `.b32.i2p`)

To see which I2P peers your node is connected to, use `blokecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `blokecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `blokecoin-cli help getnodeaddresses` for details.

## Compatibility

Blokecoin Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Blokecoin Core
...
To allow future upgrades to newer versions of SAM, Blokecoin Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Blokecoin Core.
...
It's important that the nodes of a popular application like Blokecoin contribute
...
## Bundling I2P in a Blokecoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Blokecoin.
