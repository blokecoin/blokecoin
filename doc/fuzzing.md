# Fuzzing Blokecoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Blokecoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/blokecoin/blokecoin
$ cd blokecoin/
$ cmake --preset=libfuzzer
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/blokecoin/blokecoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
...
## Overview of Blokecoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/blokecoin_report.pdf) includes an external overview of the status of Blokecoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/blokecoin/blokecoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/blokecoin/blokecoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/blokecoin/blokecoin/tree/master/src/test/fuzz).
...
It is possible to specify `blokecoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`blokecoin-core/qa-assets`](https://github.com/blokecoin-core/qa-assets) repo.

To fuzz `process_message` using the [`blokecoin-core/qa-assets`](https://github.com/blokecoin-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/blokecoin-core/qa-assets
...
an instrumentalized libc++, then using it to build [Blokecoin Core dependencies
from source](../depends/README.md) and finally the Blokecoin Core fuzz binary
...
the qa-assets repo
(https://github.com/blokecoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`blokecoin-core/qa-assets`](https://github.com/blokecoin-core/qa-assets) repo.

Every single pull request submitted against the Blokecoin Core repo is automatically tested against all inputs in the [`blokecoin-core/qa-assets`](https://github.com/blokecoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Blokecoin Core more robust.
...
# Fuzzing Blokecoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Blokecoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Blokecoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Blokecoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
Blokecoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/blokecoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=blokecoin-core%20status:open).

Blokecoin Core follows its [security disclosure policy](https://blokecoincore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_blokecoin-core/latest).
