# User-space, Statically Defined Tracing (USDT) for Blokecoin Core

Blokecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Blokecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Blokecoin Core
...
Multiple tools can list the available tracepoints in a `blokecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Blokecoin Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/blokecoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/blokecoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/blokecoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/blokecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Blokecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/blokecoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/bin/blokecoind -v
