...
if [ "$RUN_TIDY" != "true" ]; then
  ...
  # See https://github.com/blokecoin/blokecoin/pull/28359#issuecomment-1698694748
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/blokecoin-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
  ...
  if [ ! -d "$DIR_UNIT_TEST_DATA" ]; then
    ...
    ${CI_RETRY_EXE} curl --location --fail https://github.com/blokecoin-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
  fi
fi
...
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/Blokecoin"
else
  echo > "${HOME}/.blokecoin"
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/blokecoin-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target blokecoin-tidy-tests "$MAKEJOBS"
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libblokecoin-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
  ...
  run_iwyu() {
    ...
    python3 "/include-what-you-use/iwyu_tool.py" \
             ...
             -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/blokecoin.core.imp" \
