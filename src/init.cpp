 ...
 */
static const char* BITCOIN_PID_FILENAME = "blokecoind.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-prune=<n>", strprintf("Reduce storage requirements by enabling pruning (deleting) of old blocks. This allows the pruneblokechain RPC to be called to delete specific blocks and enables automatic pruning of old blocks if a target size in MiB is provided. This mode is incompatible with -txindex. "
            "Warning: Reverting this setting requires re-downloading the entire blokechain. "
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if blokecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if blokecoin is started while in a temporary directory.\n",
    ...
    if (!do_reindex && !do_reindex_chainstate && chain_active_height <= 1) {
        uint64_t assumed_chain_bytes{chainparams.AssumedBlokechainSize() * 1024 * 1024 * 1024};
        ...
        if (!CheckDiskSpace(args.GetBlocksDirPath(), additional_bytes_needed)) {
            InitWarning(strprintf(_(
                ...
                chainparams.AssumedBlokechainSize()
            ));
        }
    }
...
bool StartIndexBackgroundSync(NodeContext& node)
{
    ...
    if (indexes_start_block) {
        ...
        if (!chainman.m_blockman.CheckBlockDataAvailability(*index_chain.Tip(), *Assert(start_block))) {
            return InitError(Untranslated(strprintf("%s best block of the index goes beyond pruned data. Please disable the index or reindex (which will download the whole blokechain again)", older_index_name)));
        }
    }
