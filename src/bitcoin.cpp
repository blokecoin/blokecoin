// Copyright (c) 2025 The Blokecoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries blokecoin-node, blokecoin-gui.
  -M, --monolithic       Run monolithic binaries blokecoind, blokecoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'blokecoin-qt [ARGS]' or 'blokecoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'blokecoind [ARGS]' or 'blokecoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'blokecoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'blokecoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'blokecoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_blokecoin [ARGS]'.
  chainstate [ARGS] Run blokecoin kernel chainstate util, equivalent to running 'blokecoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_blokecoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_blokecoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "blokecoin-gui" : "blokecoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "blokecoin-node" : "blokecoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("blokecoin-cli");
            // Since "blokecoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("blokecoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("blokecoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_blokecoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("blokecoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_blokecoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_blokecoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("blokecoin-util");
        } else {
...
//! Execute the specified blokecoind, blokecoin-qt or other command line in `args`
...
//!             be a relative path to a blokecoind, blokecoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/blokecoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/blokecoin/blokecoin/pull/31375#discussion_r1861814807)
