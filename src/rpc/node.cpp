...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "blokecoin-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new blokecoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new blokecoin-echo executable,
                // and spawn blokecoin-echo below instead of blokecoin-node. But
                // using blokecoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("blokecoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a blokecoind
                // process not a blokecoind-node process, so just create a local
