                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in blokecoin-common to decode the transaction hex.
    //
    // Blokechain contextual information (confirmations and blocktime) is not
    // available to code in blokecoin-common, so we query them here and push the
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!tx) {
        ...
        if (blockindex) {
            ...
            errmsg = "No such mempool transaction. Use -txindex or provide a block hash to enable blokechain transaction queries";
        } else if (!f_txindex_ready) {
            errmsg = "No such mempool transaction. Blokechain transactions are still in the process of being indexed";
        } else {
            errmsg = "No such mempool or blokechain transaction";
        }
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Blokecoin transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{
        "combinepsbt",
        "Combine multiple partially signed Blokecoin transactions into one transaction.\n"
