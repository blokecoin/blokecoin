...
const char * const BITCOIN_CONF_FILENAME = "blokecoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //blokecoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Blokecoin
    //   new: C:\Users\Username\AppData\Local\Blokecoin
    // macOS: ~/Library/Application Support/Blokecoin
    // Unix-like: ~/.blokecoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Blokecoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Blokecoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Blokecoin";
#else
    // Unix-like
    return pathRet / ".blokecoin";
#endif
#endif
}
