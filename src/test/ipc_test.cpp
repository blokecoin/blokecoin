// Copyright (c) 2023 The Blokecoin Core developers
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_blokecoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_blokecoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_blokecoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_blokecoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_Blokecoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_blokecoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("blokecoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("blokecoin_sock1_XXXXXX")),
    };
