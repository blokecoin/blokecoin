...
RPCHelpMan importprunedfunds()
{
    return RPCHelpMan{
        "importprunedfunds",
        "Imports funds without rescan. Corresponding address or script must previously be included in wallet. Aimed towards pruned wallets. The end-user is responsible to import additional transactions that subsequently spend the imported outputs or rescan after the point in the blokechain the transaction is included.\n",
                {
...
RPCHelpMan importdescriptors()
{
    return RPCHelpMan{
        "importdescriptors",
        "Import descriptors. This will trigger a rescan of the blokechain based on the earliest timestamp of all descriptors being imported. Requires a new wallet backup.\n"
                            ...
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    ...
                                    {"timestamp", RPCArg::Type::NUM, RPCArg::Optional::NO, "Time from which to start rescanning the blokechain for this descriptor, in " + UNIX_EPOCH_TIME + "\n"
                                        "Use the string \"now\" to substitute the current synced blokechain time.\n"
                                        "\"now\" can be specified to bypass scanning, for outputs which are known to never have been used, and\n"
                                        "0 can be specified to scan the entire blokechain. Blocks up to 2 hours before the earliest timestamp\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& main_request) -> UniValue
{
    ...
    // Rescan the blokechain using the lowest timestamp
    if (rescan) {
                ...
                if (scanned_time <= GetImportTimestamp(request, now) || results.at(i).exists("error")) {
                    ...
                    if (pwallet->chain().havePruned()) {
                        error_msg += strprintf(" This error could be caused by pruning or data corruption "
                                "(see blokecoind log for details) and could be dealt with by downloading and "
                                "rescanning the relevant blocks (see -reindex option and rescanblokechain RPC).");
