...
RPCHelpMan walletpassphrase()
{
    return RPCHelpMan{
        ...
        "Stores the wallet decryption key in memory for 'timeout' seconds.\n"
                "This is needed prior to performing transactions related to private keys such as sending blokecoins\n"
            "\nNote:\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (pwallet->IsScanningWithPassphrase()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: the wallet is currently being used to rescan the blokechain for related transactions. Please call `abortrescan` before changing the passphrase.");
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (pwallet->IsScanningWithPassphrase()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: the wallet is currently being used to rescan the blokechain for related transactions. Please call `abortrescan` before locking the wallet.");
    }
...
RPCHelpMan encryptwallet()
{
    return RPCHelpMan{
        ...
        "will require the passphrase to be set prior to making these calls.\n"
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending blokecoin\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (pwallet->IsScanningWithPassphrase()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: the wallet is currently being used to rescan the blokechain for related transactions. Please call `abortrescan` before encrypting the wallet.");
    }
