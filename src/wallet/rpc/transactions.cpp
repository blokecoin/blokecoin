        ...
        "If a label name is provided, this will return only incoming transactions paying to addresses with the specified label.\n"
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::STR, "address",  /*optional=*/true, "The blokecoin address of the transaction (not returned if the output does not have an address, e.g. OP_RETURN null data)."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            {RPCResult::Type::OBJ, "", "",
                            {
                                {RPCResult::Type::STR, "address", /*optional=*/true, "The blokecoin address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
RPCHelpMan rescanblokechain()
{
    return RPCHelpMan{
        "rescanblokechain",
        "Rescan the local blokechain for wallet related transactions.\n"
                ...
                RPCExamples{
                    HelpExampleCli("rescanblokechain", "100000 120000")
            + HelpExampleRpc("rescanblokechain", "100000, 120000")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
    ...
    LOCK(pwallet->m_relock_mutex);
    {
        ...
        if (!pwallet->chain().hasBlocks(pwallet->GetLastBlockHash(), start_height, stop_height)) {
            if (pwallet->chain().havePruned() && pwallet->chain().getPruneHeight() >= start_height) {
                throw JSONRPCError(RPC_MISC_ERROR, "Can't rescan beyond pruned data. Use RPC call getblokechaininfo to determine your pruned height.");
            }
...
RPCHelpMan abortrescan()
{
    return RPCHelpMan{"abortrescan",
                "Stops current wallet rescan triggered by an RPC call, e.g. by a rescanblokechain call.\n"
                "Note: Use \"getwalletinfo\" to query the scanning progress.\n",
                {},
                RPCResult{RPCResult::Type::BOOL, "", "Whether the abort was successful"},
                RPCExamples{
            "\nImport a private key\n"
            + HelpExampleCli("rescanblokechain", "") +
