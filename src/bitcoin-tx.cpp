...
static void SetupBlokecoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " blokecoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The blokecoin-tx tool is used for creating and modifying blokecoin transactions.\n\n"
                "blokecoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded blokecoin transaction, or with \"-create [commands]\" to create a hex-encoded blokecoin transaction.\n"
                "\n"
                "Usage: blokecoin-tx [options] <hex-tx> [commands]\n"
                "or:    blokecoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded blokecoin transaction
