...
We aim to cut a regular release every 3-4 months, approximately twice as frequent as major Blokecoin Core releases. Every second release should be published one month before the feature freeze of the next major Blokecoin Core release, allowing sufficient time to update the library in Core.
...
3. After the PR is merged, tag the commit, and push the tag:
   ...
   git push git@github.com:blokecoin-core/secp256k1.git v$MAJOR.$MINOR.$PATCH
   ```
...
6. Send an announcement email to the blokecoin-dev mailing list.
...
1. If there's no maintenance branch `$MAJOR.$MINOR`, create one:
   ...
   git push git@github.com:blokecoin-core/secp256k1.git $MAJOR.$MINOR
   ```
...
7. Send an announcement email to the blokecoin-dev mailing list.
