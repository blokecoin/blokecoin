...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking blokecoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before blokecoin
    // code in the destructors are run. This way if the blokecoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In blokecoin core, running these callbacks in the right order is
