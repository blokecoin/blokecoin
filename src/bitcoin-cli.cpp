...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: blokecoin-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to blokecoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The blokecoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: blokecoin-cli [options] <command> [params]\n"
                "or:    blokecoin-cli [options] -named <command> [name=value]...\n"
                "or:    blokecoin-cli [options] help\n"
                "or:    blokecoin-cli [options] help <command>\n"
                "\n";
...
struct AddrinfoRequestHandler : BaseRequestHandler {
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires blokecoind server to be running v22.0 and up");
        }
...
struct GetinfoRequestHandler : BaseRequestHandler {
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        ...
        result.push_back(JSONRPCRequestObj("getblokechaininfo", NullUniValue, ID_BLOCKCHAININFO));
    ...
    UniValue ProcessReply(const UniValue &batch_in) override
    {
        ...
        // Errors in getnetworkinfo() and getblokechaininfo() are fatal, pass them on;
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (const auto res{ToIntegral<uint8_t>(args.at(0))}) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: blokecoin-cli -netinfo help", args.at(0)));
            }
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    m_outbound_only_selected = true;
                } else if (n) {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: blokecoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: blokecoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires blokecoind server to be running v0.21.0 and up");
        }
    ...
    const std::string m_help_doc{
        ...
        "> blokecoin-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> blokecoin-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> blokecoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> blokecoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title blokecoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> blokecoin-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the blokecoind server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"blokecoin-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            ...
            strPrint += " (run \"blokecoin-cli -h\" for help or \"blokecoin-cli listwallets\" to see which wallets are currently loaded).";
        }
    } else {
