...
// Copyright (c) 2009-2022 The Blokecoin Core developers
...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/blokecoin.conf are parsed in qt/blokecoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see blokecoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(interfaces::Init& init, ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (blokecoind) is a headless program that connects to the Blokecoin network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the Blokecoin network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for blokecoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect blokecoind signal handlers
    noui_connect();
