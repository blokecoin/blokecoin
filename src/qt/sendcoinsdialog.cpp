...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        connect(ui->customFee, &BlokecoinAmountField::valueChanged, this, &SendCoinsDialog::coinControlUpdateLabels);
#if (QT_VERSION >= QT_VERSION_CHECK(6, 7, 0))
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Blokecoin Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    processSendCoinsReturn(prepareStatus,
        BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), m_current_transaction->getTransactionFee()));
    ...
    for (const SendCoinsRecipient &rcp : m_current_transaction->getRecipients())
    {
        ...
        QString amount = BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Blokecoin Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this transaction or create a Partially Signed Blokecoin Transaction (PSBT), which you can save or copy and then sign with, e.g., an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    } else {
    ...
    if(txFee > 0)
    {
        ...
        question_string.append(BlokecoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    for (const BlokecoinUnit u : BlokecoinUnits::availableUnits()) {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(BlokecoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    question_string.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
        .arg(BlokecoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->wallet().getDefaultMaxTxFee()));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(tr("%1/kvB").arg(BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value())));
    }
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(tr("%1/kvB").arg(BlokecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK())));
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Blokecoin address"));
        }
        else // Valid address
        {
