...
Intro::Intro(QWidget *parent, int64_t blokechain_size_gb, int64_t chain_state_size_gb) :
    QDialog(parent, GUIUtil::dialog_flags),
    ui(new Ui::Intro),
    m_blokechain_size_gb(blokechain_size_gb),
...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        Intro intro(nullptr, Params().AssumedBlokechainSize(), Params().AssumedChainStateSize());
        intro.setDataDirectory(dataDir);
        intro.setWindowIcon(QIcon(":icons/blokecoin"));
        did_show_intro = true;
     ...
     * override -datadir in the blokecoin.conf file in the default data directory
     * (to be consistent with blokecoind behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::setStatus(int status, const QString &message, quint64 bytesAvailable)
{
    ...
    if(status == FreespaceChecker::ST_ERROR)
    {
        ...
        if (ui->prune->isEnabled() && m_prune_checkbox_is_default) {
            ui->prune->setChecked(m_bytes_available < (m_blokechain_size_gb + m_chain_state_size_gb + 10) * GB_BYTES);
        }
        UpdateFreeSpaceLabel();
    }
...
{
    m_required_space_gb = m_blokechain_size_gb + m_chain_state_size_gb;
    QString storageRequiresMsg = tr("At least %1 GB of data will be stored in this directory, and it will grow over time.");
    if (prune_checked && m_prune_target_gb <= m_blokechain_size_gb) {
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Blokecoin block chain.").arg(CLIENT_NAME) + " " +
