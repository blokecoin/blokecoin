// Copyright (c) 2016-2021 The Blokecoin Core developers
...
void RPCNestedTests::rpcNestedTests()
{
    ...
    std::string result2;
    std::string filtered;
    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo()[chain]", &filtered); //simple result filtering with path
    QVERIFY(result=="main");
    QVERIFY(filtered == "getblokechaininfo()[chain]");
    ...
    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo");
    QVERIFY(result.starts_with("{"));

    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo()");
    QVERIFY(result.starts_with("{"));

    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo "); //whitespace at the end will be tolerated
    QVERIFY(result.starts_with("{"));

    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo()[\"chain\"]"); //Quote path identifier are allowed, but look after a child containing the quotes in the key
    QVERIFY(result == "null");
...
#endif
    QVERIFY_EXCEPTION_THROWN(RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo() .\n"), std::runtime_error); //invalid syntax
    QVERIFY_EXCEPTION_THROWN(RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo() getblokechaininfo()"), std::runtime_error); //invalid syntax
    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo("); //tolerate non closing brackets if we have no arguments
    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo()()()"); //tolerate non command brackets
    QVERIFY_EXCEPTION_THROWN(RPCConsole::RPCExecuteCommandLine(m_node, result, "getblokechaininfo(True)"), UniValue); //invalid argument
    QVERIFY_EXCEPTION_THROWN(RPCConsole::RPCExecuteCommandLine(m_node, result, "a(getblokechaininfo(True))"), UniValue); //method not found
