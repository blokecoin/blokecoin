// Copyright (c) 2011-2022 The Blokecoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <blokecoin-build-config.h> // IWYU pragma: keep

#include <qt/blokecoingui.h>

#include <qt/blokecoinunits.h>
...
const std::string BlokecoinGUI::DEFAULT_UIPLATFORM =
...
BlokecoinGUI::BlokecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BlokecoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BlokecoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BlokecoinGUI::showModalOverlay);
...
void BlokecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Blokecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and blokecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BlokecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BlokecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BlokecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Blokecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Blokecoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Blokecoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Blokecoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a blokecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Blokecoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BlokecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BlokecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BlokecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BlokecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BlokecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BlokecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BlokecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BlokecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BlokecoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BlokecoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BlokecoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BlokecoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BlokecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BlokecoinGUI::showDebugWindow);
}

void BlokecoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BlokecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BlokecoinGUI::setCurrentWalletBySelectorIndex);
...
void BlokecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BlokecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BlokecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BlokecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BlokecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BlokecoinGUI::enableHistoryAction(bool privacy)
{
...
void BlokecoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BlokecoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BlokecoinGUI::removeWallet);
...
WalletController* BlokecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void BlokecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BlokecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BlokecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BlokecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BlokecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BlokecoinGUI::incomingTransaction);
    connect(this, &BlokecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BlokecoinGUI::removeWallet(WalletModel* walletModel)
{
...
void BlokecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BlokecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BlokecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BlokecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BlokecoinGUI::createTrayIcon()
{
...
void BlokecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BlokecoinGUI::toggleHidden);
...
void BlokecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BlokecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BlokecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BlokecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BlokecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BlokecoinGUI::openClicked()
{
...
void BlokecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BlokecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BlokecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BlokecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BlokecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BlokecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BlokecoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BlokecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Blokecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BlokecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BlokecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BlokecoinGUI::updateHeadersSyncProgressLabel()
{
...
void BlokecoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BlokecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BlokecoinGUI::quitRequested);
...
void BlokecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BlokecoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &BlokecoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BlokecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BlokecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BlokecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BlokecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void BlokecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BlokecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BlokecoinGUI::incomingTransaction(const QString& date, BlokecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BlokecoinUnits::formatWithUnit(unit, amount, true));
...
void BlokecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BlokecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BlokecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BlokecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BlokecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BlokecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BlokecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BlokecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BlokecoinGUI::updateWindowTitle()
{
...
void BlokecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BlokecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BlokecoinGUI::detectShutdown()
{
...
void BlokecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BlokecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BlokecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BlokecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BlokecoinGUI::subscribeToCoreSignals()
{
...
void BlokecoinGUI::unsubscribeFromCoreSignals()
{
...
bool BlokecoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BlokecoinUnit> units = BlokecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BlokecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BlokecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BlokecoinUnit u : BlokecoinUnits::availableUnits()) {
        menu->addAction(BlokecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BlokecoinUnit newUnits)
{
    setText(BlokecoinUnits::longName(newUnits));
}
