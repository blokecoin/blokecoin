This directory contains the source code for the Blokecoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Blokecoin UI.
...
- ClientModel is used by the main application `blokecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a blokecoin:... link from a browser).
...
* UI elements like BlokecoinAmountField, which inherit from QWidget.
* `blokecoinstrings.cpp`: automatically generated
* `blokecoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/blokecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `blokecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `blokecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `blokecoin-qt`
9. You're all set! Start developing, building, and debugging the Blokecoin Core GUI
