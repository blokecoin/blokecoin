...
/** Call node.updateRwSetting() with Blokecoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Blokecoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/blokecoin/blokecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because blokecoin 22.x will not try to read these.
...
//! Convert enabled/size values to blokecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from blokecoin -prune setting.
...
//! Get pruning size value to show in GUI from blokecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBlokecoinUnit")) {
        settings.setValue("DisplayBlokecoinUnit", QVariant::fromValue(BlokecoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayBlokecoinUnit");
    if (unit.canConvert<BlokecoinUnit>()) {
        m_display_blokecoin_unit = unit.value<BlokecoinUnit>();
    } else {
        m_display_blokecoin_unit = BlokecoinUnit::BTC;
        settings.setValue("DisplayBlokecoinUnit", QVariant::fromValue(m_display_blokecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if blokecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_blokecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BlokecoinUnit>() == m_display_blokecoin_unit) return;
    m_display_blokecoin_unit = new_unit.value<BlokecoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBlokecoinUnit", QVariant::fromValue(m_display_blokecoin_unit));
    Q_EMIT displayUnitChanged(m_display_blokecoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/blokecoin/blokecoin/pull/8273
    ...
    // (https://github.com/blokecoin-core/gui/issues/567).
