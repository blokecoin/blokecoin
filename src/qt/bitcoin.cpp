...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<BlokecoinUnit>("BlokecoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. blokecoin_de.qm (shortcut "de" needs to be defined in blokecoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. blokecoin_de_DE.qm (shortcut "de_DE" needs to be defined in blokecoin.qrc)
...
static const char* qt_argv = "blokecoin-qt";

BlokecoinApplication::BlokecoinApplication()
...
void BlokecoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BlokecoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BlokecoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BlokecoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BlokecoinApplication::createOptionsModel(bool resetSettings)
{
...
void BlokecoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BlokecoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BlokecoinGUI::quitRequested, this, &BlokecoinApplication::requestShutdown);
...
void BlokecoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BlokecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BlokecoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BlokecoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BlokecoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BlokecoinApplication::handleRunawayException);
    connect(this, &BlokecoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BlokecoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BlokecoinApplication::parameterSetup()
{
...
void BlokecoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BlokecoinApplication::requestInitialize()
{
...
void BlokecoinApplication::requestShutdown()
{
...
void BlokecoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // blokecoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BlokecoinGUI::handlePaymentRequest);
            connect(window, &BlokecoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BlokecoinApplication::handleRunawayException(const QString &message)
{
...
void BlokecoinApplication::handleNonFatalException(const QString& message)
{
...
WId BlokecoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BlokecoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BlokecoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(blokecoin);
    Q_INIT_RESOURCE(blokecoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see blokecoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see blokecoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse blokecoin.conf, determine network, switch to network specific
    ...
    // blokecoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
