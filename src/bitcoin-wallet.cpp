// Copyright (c) 2016-present The Blokecoin Core developers
...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.GetBoolArg("-version", false)) {
        std::string strUsage = strprintf("%s blokecoin-wallet utility version", CLIENT_NAME) + " " + FormatFullVersion() + "\n";

        if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "blokecoin-wallet is an offline tool for creating and interacting with " CLIENT_NAME " wallet files.\n\n"
                "By default blokecoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n\n"
                "To change the target wallet, use the -datadir, -wallet and (test)chain selection arguments.\n"
                "\n"
                "Usage: blokecoin-wallet [options] <command>\n"
                "\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
MAIN_FUNCTION
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `blokecoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
